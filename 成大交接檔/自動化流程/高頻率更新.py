...
need_normalize = ['週','工作天數','休假天數','前1週','前2週','前3週','前4週','數量','每月第幾週','相似日','滑動平均',
                  '年','最大溫差','最大平均溫度差','相似資料','前1週人數','前1週最多人', '前1週平均人數']
...
room = "五病房"
# 先設置基本輸入
basic_input = ['週','前1週','前2週','每月第幾週']
# 要測試的變數
other_input = ['前3週','前4週','休假天數','最大溫差','相似資料','前1週人數','前1週最多人','前1週開刀人數','前1週開刀最多人']
# 方法:每次加入一個變數，觀察是否會變更好，如果都沒有的話就停止
# 設置最佳組合dataframe
best_input = ['週','前1週','前2週','每月第幾週']
...
def main(part_no,temp_input,room):
    remove_score = 0
   
    
    need_normalize = ['週','工作天數','休假天數','前1週','前2週','前3週','前4週','數量','每月第幾週','相似日','滑動平均',
                  '年','最大溫差','最大平均溫度差','相似資料','前1週人數','前1週最多人', '前1週平均人數','前1週開刀人數','前1週開刀最多人']
...
def renew_column(part_no,room):
#     filename = "K80004044"
    filename = part_no
    first_time = 1
    stop = False
    temp_input = other_input
    #     第一次使用基本輸入
    best_dataframe = main(filename,basic_input,room)
    best_column ="最佳解欄位"
    while stop != True:
        best_column ="最佳解欄位"
        for column in temp_input:
            basic_input.append(column) 
            temp_dataframe = main(filename,basic_input,room)
            basic_input.remove(column)
            if ((temp_dataframe['P(RMSE)'].values[0] + temp_dataframe['P(Mae)'].values[0]) - (best_dataframe['P(RMSE)'].values[0] + best_dataframe['P(Mae)'].values[0]) ) < 0:
                best_dataframe = temp_dataframe
                best_column = column
        if best_column == "最佳解欄位":
            stop = True
        else:
            basic_input.append(best_column) 
            temp_input.remove(best_column)
    return best_dataframe,basic_input
...
# best_dataframe,basic_input = renew_column("K80004044","五病房")
