...
def main(part_no,room):
    ...
    if (len(data_2019) < 13) | (len(data_2020) < 13) | (len(data_2021) < 13):
        return -1000
...
# 儲存模型
    joblib.dump(arima_model,f'./1個月預測Arima模型/{room}/{part_no}.pkl')
# 儲存所有預測
    prediction = pd.DataFrame()

    # 讀取模型
    auto_arima = joblib.load(f"./1個月預測Arima模型/{room}/{part_no}.pkl")
    ...
    for i in range(0,12-len_outlier,1):
        print(i)
