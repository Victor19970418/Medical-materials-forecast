...
def main(room,part_no):
    ...
    check_part_nos= check_part_nos[check_part_nos['料號'] == part_no]
    check_part_nos['預測值'] = 0
    check_part_nos['預測方式'] = "暫無判斷"
    ...
    for i in range(len(check_part_nos)):
        ...
        check_part_nos.loc[i:i,'預測方式'] = method_type
    ...
    for i in range(len(wrong)):
        ...
        if temp['紀錄週數'].values[0] >= 40:
            ...
            check_part_nos.loc[check_part_nos['料號']==temp['料號'].values[0],'預測值'] = answer
        check_part_nos.loc[check_part_nos['料號']==temp['料號'].values[0],'預測方式'] = "基線"
    ...
    # check_part_nos['預測方式'] = "測試"
    check_part_nos.to_csv(f"./單一衛材預測結果/{part_no}.csv",encoding='utf_8_sig')
